...
def train(config, epoch, num_epoch, epoch_iters, base_lr,
    ...
    for i_iter, batch in enumerate(trainloader, 0):
        ...
        if torch.isnan(loss) or torch.isinf(loss):
            logging.warning('NaN/Inf loss detected at epoch {} iter {}, skipping batch'.format(epoch, i_iter))
            model.zero_grad()
            continue
...
def validate(config, testloader, model, writer_dict):
    ...
    for i in range(nums):
        ...
        # Per-class IoU with class names for clear diagnostics
        per_class_msg = 'Output[{}] mIoU: {:.4f}'.format(i, mean_IoU)
        for ci in range(len(IoU_array)):
            cname = class_names[ci] if ci < len(class_names) else 'class_{}'.format(ci)
            per_class_msg += ' | {}: {:.4f}'.format(cname, IoU_array[ci])
        logging.info(per_class_msg)
