...
CLASS_NAMES = ['background', 'powerline', 'tower_wooden', 'tower_lattice', 'tower_tucohy']
NUM_CLASSES = 5
# B-IoU和Relaxed IoU仅在前景类上计算（背景边界无意义）
FOREGROUND_CLASSES = [1, 2, 3, 4]  # powerline, tower_wooden, tower_lattice, tower_tucohy
...
def compute_boundary_iou(pred, label, num_classes, radius=2):
    ...
    for cls_idx in FOREGROUND_CLASSES:
        ...
        # 边界提取失败（区域太小被erosion完全腐蚀）→ 降级为标准IoU
        if gt_boundary.sum() == 0:
            intersection = np.sum(gt_mask & pred_mask)
            union = np.sum(gt_mask | pred_mask)
            biou_per_class[cls_idx] = intersection / np.maximum(union, 1e-10)
            continue
...
def evaluate(model, dataset, device, num_classes):
    ...
    for idx in tqdm(range(len(dataset)), desc='Evaluating'):
        ...
        if isinstance(output, (list, tuple)):
            output = output[1]  # 主分割输出（[0]=P分支辅助, [1]=主输出, [2]=D分支边缘检测）
        ...
        # 逐图像计算B-IoU和Relaxed IoU（需要2D空间信息，不能展平）
        if valid_mask.sum() > 0:
            biou_per_class = compute_boundary_iou(pred, label, num_classes)
            riou_per_class = compute_relaxed_iou(pred, label, num_classes)
            biou_accum.append(biou_per_class)
            riou_accum.append(riou_per_class)

    metrics = compute_metrics(confusion)

    # 聚合B-IoU：逐图像前景类平均，存储为完整num_classes数组（背景=NaN）
    if biou_accum:
        biou_arr = np.array(biou_accum)  # (N_images, num_classes)
        biou_fg = biou_arr[:, FOREGROUND_CLASSES]  # 仅前景类
        biou_fg_mean = np.nanmean(biou_fg, axis=0)  # per-class mean (2 elements)
        biou_full = np.full(num_classes, np.nan)
        for idx, cls in enumerate(FOREGROUND_CLASSES):
            biou_full[cls] = biou_fg_mean[idx]
        metrics['biou'] = biou_full
        metrics['mean_biou'] = np.nanmean(biou_fg_mean)

    # 聚合Relaxed IoU：逐图像前景类平均，存储为完整num_classes数组（背景=NaN）
    if riou_accum:
        riou_arr = np.array(riou_accum)  # (N_images, num_classes)
        riou_fg = riou_arr[:, FOREGROUND_CLASSES]  # 仅前景类
        riou_fg_mean = np.nanmean(riou_fg, axis=0)  # per-class mean (2 elements)
        riou_full = np.full(num_classes, np.nan)
        for idx, cls in enumerate(FOREGROUND_CLASSES):
            riou_full[cls] = riou_fg_mean[idx]
        metrics['riou'] = riou_full
        metrics['mean_riou'] = np.nanmean(riou_fg_mean)
...
def plot_training_curves(log_path, output_dir):
    ...
    val_mious = []
    class_ious_history = defaultdict(list)  # class_name -> [ious...]
    ...
    for line in lines:
        ...
        if 'Output[1] mIoU:' in line:
            try:
                ...
                # 通用解析 per-class IoU：按 | 分割，每段按 : 拆 key/value
                for segment in parts.split('|')[1:]:
                    segment = segment.strip()
                    if ':' in segment:
                        key, val = segment.split(':', 1)
                        class_ious_history[key.strip()].append(float(val.strip()))
            except (IndexError, ValueError):
                pass
    ...
    # 绘制所有有数据的类别
    colors = ['g--', 'r--', 'm--', 'c--', 'y--']
    for i, (cname, cious) in enumerate(class_ious_history.items()):
        ax1.plot(range(len(cious)), cious, colors[i % len(colors)], alpha=0.7, label=cname)
    ...
    ax2 = axes[1]
    # 改画所有前景类 IoU 的方差/离散度而非固定 2 类减法
    if len(class_ious_history) >= 2:
        keys = list(class_ious_history.keys())
        # 取所有非背景类的 IoU
        fg_keys = [k for k in keys if 'background' not in k.lower()]
        if len(fg_keys) >= 2:
            # 画最低和最高前景类 IoU 的 gap
            lo_ious = class_ious_history[fg_keys[0]]
            hi_ious = class_ious_history[fg_keys[-1]]
            min_len = min(len(lo_ious), len(hi_ious))
            gaps = [hi_ious[i] - lo_ious[i] for i in range(min_len)]
            ax2.fill_between(range(len(gaps)), gaps, alpha=0.3, color='purple')
            ax2.plot(range(len(gaps)), gaps, 'purple', linewidth=2,
                     label=f'{fg_keys[-1]} - {fg_keys[0]}')
    ax2.axhline(y=0, color='gray', linestyle='-', alpha=0.3)
    ax2.set_xlabel('Validation Step')
    ax2.set_ylabel('IoU Gap')
    ax2.set_title('Foreground Class IoU Gap')
    ax2.legend()
    ax2.grid(alpha=0.3)
